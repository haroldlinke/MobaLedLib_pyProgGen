...
def Create_Start_Sub(BoardName, ResultName, ComPort, BuildOptions, InoName, SrcDir, CPUType):
        ...
        if UBound(OptParts) >= 1:
            if OptParts(0) == '--board':
                ...
                if InStr(BuildOptions, M02.BOARD_NANO_OLD) or InStr(BuildOptions, M02.BOARD_UNO_NORM) > 0:
                    ...
                    BaudRate = '115200'
                CommandStr = '"' + M30.FilePath(Find_ArduinoExe()) + '" "' + InoName + '" ' + ComPort + ' "' + BuildOptOnly + '" ' + BaudRate + '  "' + GetShortPath(M30.DelLast(M27.Get_Ardu_LibDir())) + '" ' + CPUType
    ...
    #*HL Create_Build(BoardName, fp) # M08_Fast_ARDUINO
...
def Create_Start_ESP32_Sub(ResultName, ComPort, BuildOptions, InoName, SrcDir, CPUType):
    ...
    if M30.Win10_or_newer():
        VBFiles.writeText(fp, 'CHCP 65001 >NUL', '\n')
    VBFiles.writeText(fp, '', '\n')
    VBFiles.writeText(fp, 'set ArduinoLib=%USERPROFILE%\\Documents\\Arduino\\libraries', '\n')
    VBFiles.writeText(fp, 'if not exist MyPrivateBuildScript.cmd (', '\n')
    VBFiles.writeText(fp, '       REM embedded Fast Build and Upload', '\n')
...
def Check_If_Arduino_could_be_programmed_and_set_Board_type(ComPortColumn, BuildOptColumn, BuildOptions, DeviceSignature):
    Start_Baudrate = int()
...
def Update_Compile_Time(Start=VBMissingArgument):
    #---------------------------------------------------------
    return
    # Is called by OnTime
    ...
    #    else:
    #        StatusMsg_UserForm.Set_ActSheet_Label(Format(Time - Start_Compile_Time, 'hh:mm:ss'))
...
def Compile_and_Upload_Prog_to_Arduino(InoName, ComPortColumn, BuildOptColumn, SrcDir):
    ComPort = String()
    ...
    #*HL U01.Unload(UserForm_Options)
    #*HL StatusMsg_UserForm.ShowDialog(Replace(Get_Language_Str('Programmiere #1# Arduino'), "#1#", ArduName) + vbCr + FileNameExt(InoName), '...')
    #*HL Update_Compile_Time(True)
    
    """
    
    Fn_result, BuildOptions, DeviceSignature = Check_If_Arduino_could_be_programmed_and_set_Board_type(ComPortColumn, BuildOptColumn, BuildOptions, DeviceSignature)
    if Fn_result == False:
    ...
    if M02.Get_BoardTyp() == 'ESP32' or M02.Get_BoardTyp() == 'PICO':
        if Mode == 'Right':
            ...
            if M25.Page_ID == 'DCC' or M25.Page_ID == 'CAN':
                ...
                P01.MsgBox('Error: The ESP32 support for \'' + M25.Page_ID + '\' is not finished yet', vbInformation, 'ESP32 support not finished')
                return fn_return_value
        TextColor = vbYellow
    if DeviceSignature == 2004561:
        CPUType = 'atmega4809'
    ...
    if Dir(ResFile) != '':
        ...
        #Show_Status_for_a_while(Get_Language_Str('Programm erfolgreich hochgeladen. Kompilieren und Hochladen dauerte ') + Format(Time - Start, 'hh:mm:ss'), '00:02:00')
        fn_return_value = True
    return fn_return_value

def Compile_and_Upload_LED_Prog_to_Arduino():
    #------------------------------------------------------------------
    if True: #*HL Upload_the_Right_Arduino_Prog_if_needed():
        fn_return_value = Compile_and_Upload_Prog_to_Arduino(M02.InoName_LED, M25.COMPort_COL, M25.BUILDOP_COL, P01.ThisWorkbook.Path + '\\' + M02.Ino_Dir_LED)
    return fn_return_value
...
def Install_Libraries(LibNames=VBMissingArgument):
    ...
    if Dir(ResFile) != '':
        ...
        Debug.Print('Compile and upload duaration: ' + P01.Format(Time - Start, 'hh:mm:ss'))
