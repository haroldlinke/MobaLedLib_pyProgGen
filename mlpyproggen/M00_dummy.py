...
def ShellAndWait(ShellCommand, TimeOutSeconds, ShellWindowState, BreakKey):
    ...
    if Trim(ShellCommand) == vbNullString:
        fn_return_value = ShellAndWaitResult.InvalidParameter
    ...
    while not (WaitRes == __WAIT_OBJECT_0):
        ...
        if (WaitRes == __WAIT_ABANDONED):
            ...
            fn_return_value = ShellAndWaitResult.SysWaitAbandoned
            break
        elif (WaitRes == __WAIT_OBJECT_0):
            # Successful completion
            fn_return_value = ShellAndWaitResult.Success
            break
            ...
            if ms > 0:
                ...
                if ElapsedTime > ms:
                    fn_return_value = ShellAndWaitResult.Timeout
                    break
                else:
...
assert False, '# UNTRANSLATED VB LINE #24 [#End If]'
