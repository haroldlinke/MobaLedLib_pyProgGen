...
def Create_Build_Arduino(fp):
    ...
    if Get_User_std_Arduino_Lib_Ver() == '':
        VBFiles.writeText(fp, 'robocopy "%aHome%\\hardware\\arduino\\avr" "' + PackageDestDir + '" /mir /s >nul', '\n')
    VBFiles.writeText(fp, 'xcopy ' + GetShortPath(ThisWorkbook.Path) + '\\LEDs_AutoProg\\boards.local.txt "' + PackageDestDir + '\\" /d /y >nul', '\n')
    ...
    VBFiles.writeText(fp, '     -hardware "' + GetShortPath(Environ(Env_USERPROFILE) + AppLoc_Ardu + 'packages') + '" ^', '\n')
...
def Create_Build_Pico(fp):
    ...
    VBFiles.writeText(fp, 'call :short packages "%USERPROFILE%' + AppLoc_Ardu + 'packages"', '\n')
    VBFiles.writeText(fp, 'if not exist "%aTemp%"  md "%aTemp%"', '\n')
    VBFiles.writeText(fp, 'if not exist "%aCache%" md "%aCache%"', '\n')
    VBFiles.writeText(fp, '', '\n')
    Create_Packages_Dir_if_not_Available()
