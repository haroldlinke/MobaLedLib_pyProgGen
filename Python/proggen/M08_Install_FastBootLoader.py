...
def __Install_ArduinoISP_to_Right_Arduino():
    ...
    CreateFolder(DstDir)
    if not FileCopy_with_Check(DstDir, InoName, SrcDir + InoName):
        return fn_return_value
    if Compile_and_Upload_Prog_to_Arduino(InoName, COMPrtR_COL, BUILDOpRCOL, DstDir):
...
def Create_WriteFastBootloader_cmd(SrcDir):
    ...
    VBFiles.writeText(fp, 'REM', '\n')
    VBFiles.writeText(fp, 'REM This file was automatically generated by the program ' + ThisWorkbook.Name + ' ' + Prog_Version + '      by Hardi', '\n')
    VBFiles.writeText(fp, 'REM File creation: ' + Date + ' ' + Time, '\n')
    ...
    VBFiles.writeText(fp, '   "-Uflash:w:' + GetShortPath(Get_SrcDirInLib() + 'ArduinoISP\\optiboot_atmega328_Ver108.1.hex') + ':i" ^', '\n')
...
def __Write_Bootloader():
    ...
    if CmdName == '':
        return fn_return_value
    CommandStr = '"' + CmdName + '" "' + FilePath(Find_ArduinoExe()) + '"' + ' -PCOM' + Cells(SH_VARS_ROW, COMPrtR_COL)
...
def Install_FastBootloader():
    ...
    #----------------------------------
    Make_sure_that_Col_Variables_match()
    if Page_ID != 'DCC' and Page_ID != 'Selectrix':
        MsgBox(Get_Language_Str('Die schnelle Bootloader kann nur von einer DCC oder Selectrix Seite aus installiert werden.'), vbInformation, Get_Language_Str('Falsche Seite zum aktualisieren des Bootloaders ausgewählt'))
        return
    ...
    if Res:
        ...
        while 1:
            __Write_Bootloader()()
            if not (MsgBox(Get_Language_Str('Installation des Bootloaders abgeschlossen' + vbCr + vbCr + 'Soll der Bootloader auf einen weiteren Arduino geladen werden?' + vbCr + vbCr + 'Wenn ja, dann muss dieser jetzt in den linken Steckplatz gesteckt werden.' + vbCr + 'Mit \'Nein\' wird wieder das DCC/Selectrix Programm auf den rechten Nano installiert.' + vbCr + vbCr + 'Achtung der rechte Arduino darf nicht entfernt werden!'), vbYesNo + vbDefaultButton2, Get_Language_Str('Noch einen Arduino aktualisieren?')) == vbYes):
                break
        Compile_and_Upload_Prog_to_Right_Arduino()
        MsgBox(Get_Language_Str('Achtung: Die Jumper müssen unbedingt wieder entfernt werden sonst geht nichts mehr ;-(' + vbCr + 'Damit sie nicht verloren gehen können sie so eingesteckt werden, dass sie nur auf einem Pin stecken.' + vbCr + vbCr + 'Das USB Kabel sollte wieder auf den linken Arduino gesteckt werden.'), vbInformation, Get_Language_Str('Bootloader Programmierung abgeschlossen'))
